...
function addReservationToTable(reservations) {
	...
	const reservationTableParentDiv = document.getElementById(
	...
	if (reservations.length === 0) {
		...
		reservations.forEach((adventure) => {
			...
			let bookedDate = new Date(adventure.time)
				.toLocaleString('en-IN', { dateStyle: 'long', timeStyle: 'medium' })
				.replace(' at', ',');
			let adventureDetailURL = window.location.href.slice(
				0,
				window.location.href.indexOf('reservations')
			);
	...
	function displayModalEvents(event) {
		...
		modalYesBtn.addEventListener('click', () =>
			deleteAdventure(event, clickedDeleteIconHTML)
		);
	}

	async function deleteAdventure(event, clickedDeleteIconHTML) {
		const adventureId = clickedDeleteIconHTML.dataset.adventureId;
		const tableRowToBeRemovedHTML =
			clickedDeleteIconHTML.parentElement.parentElement;
		const modalHTML = document.getElementById('adventureModal');
		...
		modal.hide();
		try {
			const deleteAdventureRes = await fetch(
				`${config.backendEndpoint}/reservations`,
				{
					method: 'DELETE',
					headers: {
